...
import { updateRoute, deleteRoute, createRoute } from '../../../graphql/mutations'
...
const Buttons = ({ selectedRoute, setSelectedRoute }) => {
    ...
    const updateRte = async () => {
        ...
        try{
          const routeData = await API.graphql(graphqlOperation(updateRoute, {input: {...updateDetails}}))
    ...
    const deleteRouteWarn = async () => {
        swal ({
            text: " Are you sure that you would like to permanently delete this route?",
            icon: "warning",
    ...
    const deleteRte = async () => {
        ...
        try{
            await API.graphql(graphqlOperation(deleteRoute, {input: {...deleteDetails}}));
            setSelectedRoute()
        } catch (error) {
            console.log('error on fetching Route List', error)
        }
    ...
    return (
        ...
        <ButtonBox>
            <Button label="Add a Route" icon="pi pi-plus" onClick={handleAddRoute}
                className={"p-button-raised p-button-rounded"} /><br />
            {selectedRoute && <React.Fragment><Button label="Update Route" icon="pi pi-user-edit" onClick={updateRte}
                className={"p-button-raised p-button-rounded p-button-success"} /><br /></React.Fragment>}
            {selectedRoute && <React.Fragment><Button label="Delete Route" icon="pi pi-user-minus" onClick={deleteRouteWarn}
