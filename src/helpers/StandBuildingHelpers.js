  ...
  if (Stand) {
    ...
    for (let pull of pullStand) {
      ...
      if (pull["custName"] === chosen) {
        let ind = buildStandArray.findIndex(
          (stand) => stand[0] === pull["prodName"]
        );
        if (ind >= 0) {
...
export const createUpdateWeeklyList = (index, updatedStandorHold, chosen) => {
  let indexToFind = index;
  for (let i = 1; i < 8; ++i) {
    let ind = updatedStandorHold.findIndex(
      (stand) =>
        stand["dayNum"] === i.toString() &&
        stand["prodName"] === indexToFind &&
        stand["custName"] === chosen
    );
    if (ind >= 0) {
      updatedStandorHold[ind]["qty"] = "0";
    }
  }
  updatedStandorHold = updatedStandorHold.filter(
    (stand) => stand["qty"] !== "0"
  );
...
export const createUpdateWeeklyStandList = (e, updatedStandorHold, chosen) => {
  ...
  let ind = updatedStanding.findIndex(
    (stand) =>
      stand["dayNum"] === intTarg[1] &&
      stand["prodName"] === indexToFind &&
      stand["custName"] === chosen
  );
  if (ind >= 0) {
