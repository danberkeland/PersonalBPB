...
export const todayPlus = () => {
    ...
    let tomorrow = DateTime.now().setZone("America/Los_Angeles").plus({ days: 1}) 
...
export const daysOfTheWeek = () => {
    ...
    let Sun = DateTime.now().setZone("America/Los_Angeles").plus({ days: timeDelta})
    let SunSend = Sun.toString().split('T')[0]

    let Mon = DateTime.now().setZone("America/Los_Angeles").plus({ days: (timeDelta+1)%7})
    let MonSend = Mon.toString().split('T')[0]

    let Tues = DateTime.now().setZone("America/Los_Angeles").plus({ days: (timeDelta+2)%7})  
    let TuesSend = Tues.toString().split('T')[0]

    let Wed = DateTime.now().setZone("America/Los_Angeles").plus({ days: (timeDelta+3)%7}) 
    let WedSend = Wed.toString().split('T')[0]

    let Thurs = DateTime.now().setZone("America/Los_Angeles").plus({ days: (timeDelta+4)%7})
    let ThursSend = Thurs.toString().split('T')[0]

    let Fri = DateTime.now().setZone("America/Los_Angeles").plus({ days: (timeDelta+5)%7})
    let FriSend = Fri.toString().split('T')[0]

    let Sat = DateTime.now().setZone("America/Los_Angeles").plus({ days: (timeDelta+6)%7})
